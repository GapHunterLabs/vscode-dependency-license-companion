 ...
 * a real number of distinct flagged dependency license issues -- never on install, never on
...
const KEY_SEEN_FINDINGS = 'dependencyLicenseCompanion.review.seenFindings';
const KEY_ANSWERED = 'dependencyLicenseCompanion.review.answered';
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Dependency License Companion: this has flagged ${HITS_BEFORE_PROMPT} real issues in your code -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
